...
public class Framework implements Dependent, ActionListener {
    ...
    public boolean isSetCurrent(SVarSetInterface s) {
    ...
    public SVarSetInterface selectSet(final String name) {
        int i=0;
    ...
    public int newVar(String name, final double[] d) {
	    ...
	    SVarInterface ci = cvs.byName(name);
	    if (ci!=null) {
	...
	boolean mapsHaveEqualContents(final SVarInterface v, final double[] x, final double[] y) {
    ...
    public int replaceVar(final int vi, final double[] d) {
        final SVarInterface v=cvs.at(vi);
        if (v==null) return -1;
    ...
    public SVar getVar(final int i) { return (SVar)cvs.at(i); };
    ...
    public SVarInterface getVar(final String name) { return cvs.byName(name); };
    ...
    public ScatterCanvas newScatterplot(final SVarSetInterface vs, final int v1, final int v2) {
		...
		DirectJNI.getInstance().notifyRActionListeners(action);
	...
	public MapCanvas newMap(final SVarSetInterface vs, final int v) {
		updateMarker(vs, v);
    ...
    public BarCanvas newBarchart(final SVarSetInterface vs, final int v, final int wgt) {
        updateMarker(vs,v);
        final SVarInterface theCat=vs.at(v);
...
//        addNewPlot(lc);
    ...
    public MosaicCanvas newMosaic(final SVarSetInterface vs, final int[] v) {
        if (v.length==0) return null;
        updateMarker(vs,v[0]);
        String title = "(";
    ...
    public ParallelAxesCanvas newPCP(final SVarSetInterface vs, final int[] v) {
        if (v.length==0) return null;
        updateMarker(vs,v[0]);
    ...
    public double[] getDoubleContent(final int vid) {
        final SVarInterface v=cvs.at(vid);
    ...
    /** get current dataset */
    public SVarSet getCurrentSet() { return (SVarSet)cvs; };
    ...
    public SVarSet selectSet(final int i) {
        return (i<0||i>=dataset.size())?null:(SVarSet)(cvs=dataset.get(i));
    }
