...
/** implements set of variables (aka dataset, used as data source)
    ...
    @version $Id: SVarSet.java 2185 2006-04-05 13:28:42Z wichtreyt $
*/
public class SVarSet implements SVarSetInterface {
    ...
    public int add(SVarInterface v) {  // return position or -1 on error or -2 if name exists
	if (v==null) return -1;
	String nn=v.getName();
        if (nn==null) return -1;

	if (!vars.isEmpty()) {
	    for (Iterator<SVarInterface> e=vars.listIterator(); e.hasNext();) {
		SVarInterface n=e.next();
