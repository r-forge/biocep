...
public class ParallelAxesCanvas extends BaseCanvas {
    ...
    public ParallelAxesCanvas(final int gd, final Frame f, final SVarInterface var, final SVarInterface cvar, final SMarkerInterface mark, final int type) {
        super(gd,f,mark);
     ...
     * @param mark associated marker */
    public ParallelAxesCanvas(final int gd, final Frame f, final SVarInterface var, final SMarker mark, final int type) {
        this(gd,f,new SVarInterface[]{var},mark,type);
    }
     ...
     */
    public ParallelAxesCanvas(final int gd, final Frame f, final SVarInterface[] yvs, final SMarkerInterface mark, final int type) {
        super(gd, f, mark);
    ...
    public SVarInterface getData(final int id) {
        switch(type){
            case TYPE_BOX:
    ...
    private void initOss(final SVarInterface[] yvs) {
        if(yvs.length==1){
...
class OrdStats { // get ordinal statistics to be used in boxplot
    ...
    private double medFrom(final SVarInterface v,final int[] r,final int min,final int max) {
        return (((max-min)&1)==0)
        ?v.atF(r[min+(max-min)/2])
        :((v.atF(r[min+(max-min)/2])+v.atF(r[min+(max-min)/2+1]))/2);
    };
    
    void update(final SVarInterface v, final int[] r) {
        update(v,r,r.length);
    };
    
    /* v=variable, r=ranked index as returned by getRanked, n=# of el to use */
    void update(final SVarInterface v, final int[] r, final int n) {
        lastTop=n;
        if (n<1) return;
