...
public class GUIUtilities {
	...
	public static Icon loadIcon(String iconName) {
		...
		if (icon != null)
			return icon;
		// get the icon
		icon = new ImageIcon(jEdit.class.getResource("/org/gjt/sp/jedit/icons/" + iconName));
	...
	public static EnhancedButton loadToolButton(ActionContext context, String name) {
		...
		if (shortcut1 != null || shortcut2 != null) {
			toolTip = toolTip + " (" + (shortcut1 != null ? shortcut1 : "")
					+ ((shortcut1 != null && shortcut2 != null) ? " or " : "") + (shortcut2 != null ? shortcut2 : "")
					+ ")";
		}
	...
	public static void message(Component comp, String name, Object[] args) {
		...
		JOptionPane.showMessageDialog(comp, jEdit.getProperty(name.concat(".message"), args), jEdit.getProperty(name
				.concat(".title"), args), JOptionPane.INFORMATION_MESSAGE);
	} //}}}
	...
	public static void error(Component comp, String name, Object[] args) {
		...
		JOptionPane.showMessageDialog(comp, jEdit.getProperty(name.concat(".message"), args), jEdit.getProperty(name
				.concat(".title"), args), JOptionPane.ERROR_MESSAGE);
	} //}}}
	...
	public static String input(Component comp, String name, Object[] args, Object def) {
		...
		String retVal = (String) JOptionPane.showInputDialog(comp, jEdit.getProperty(name.concat(".message"), args),
				jEdit.getProperty(name.concat(".title")), JOptionPane.QUESTION_MESSAGE, null, null, def);
		return retVal;
	} //}}}
	...
	public static String inputProperty(Component comp, String name, Object[] args, String def) {
		...
		String retVal = (String) JOptionPane.showInputDialog(comp, jEdit.getProperty(name.concat(".message"), args),
				jEdit.getProperty(name.concat(".title")), JOptionPane.QUESTION_MESSAGE, null, null, jEdit
						.getProperty(def));
		if (retVal != null)
	...
	public static int confirm(Component comp, String name, Object[] args, int buttons, int type) {
		...
		return JOptionPane.showConfirmDialog(comp, jEdit.getProperty(name + ".message", args), jEdit.getProperty(name
				+ ".title"), buttons, type);
	} //}}}
	 ...
	 * @since jEdit 4.0pre4
	 */
	public static SyntaxStyle parseStyle(String str, String family, int size, boolean color)
		...
		return new SyntaxStyle(fgColor, bgColor, new Font(family, (italic ? Font.ITALIC : 0) | (bold ? Font.BOLD : 0),
				size));
	} //}}}
	...
	static class UnixWorkaround {
		...
		UnixWorkaround(Window win, String name, Rectangle desired, int extState) {
			...
			required = new Rectangle(desired.x - adjust_x, desired.y - adjust_y, desired.width - adjust_width,
	...
	public static void saveGeometry(Window win, String name) {
		if (win instanceof Frame) {
			jEdit.setIntegerProperty(name + ".extendedState", getExtendedState((Frame) win));
		}
	...
	public static int getExtendedState(Frame frame) {
		if (OperatingSystem.hasJava14()) {
			try {
				...
				Integer extState = (Integer) meth.invoke(frame, new Object[0]);
	...
	public static void setExtendedState(Frame frame, int extState) {
		if (OperatingSystem.hasJava14()) {
			try {
				...
				meth.invoke(frame, new Object[] { new Integer(extState) });
			} catch (Exception e) {
				Log.log(Log.ERROR, GUIUtilities.class, e);
			}
		}
	...
	public static View getView(Component comp) {
		return (View) ((TaggedPanel) getComponentParent(comp, TaggedPanel.class)).getProperty("view");
	} //}}}
