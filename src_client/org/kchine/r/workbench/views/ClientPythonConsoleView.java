...
public class ClientPythonConsoleView extends DynamicView {
	...
	public ClientPythonConsoleView(String title, Icon icon, int id, RGui rgui) {
		...
		_consolePanel = new ConsolePanel(new SubmitInterface() {
			public String submit(final String expression) {
				...
				try {
					_rgui.getRLock().lock();
					try {
						org.kchine.r.server.scripting.PythonInterpreterSingleton.startLogCapture();
						org.kchine.r.server.scripting.PythonInterpreterSingleton.getInstance().exec(expression);
						return org.kchine.r.server.scripting.PythonInterpreterSingleton.getPythonStatus()+"\n";
					} catch (Exception e) {
		...
		}, "Python Expression", Color.green, "P.L", true, null);
