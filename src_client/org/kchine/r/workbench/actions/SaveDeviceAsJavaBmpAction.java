...
public class SaveDeviceAsJavaBmpAction extends AbstractAction {

	RGui _rgui;

	public SaveDeviceAsJavaBmpAction(RGui rgui) {
		super("Save as Java BMP");
		_rgui = rgui;
	}
	...
	public void actionPerformed(final ActionEvent e) {
		...
		chooser.setDialogTitle("Save Graphics as Java BMP");
			...
			new Thread(new Runnable() {
				public void run() {
					try {
						...
						byte[] result = panel.getGdDevice().getFromImageIOWriter("bmp");
						RandomAccessFile raf = new RandomAccessFile(org.kchine.rpf.PoolUtils.fixExtension(chooser.getSelectedFile(),"bmp"), "rw");
						raf.setLength(0);
						raf.write(result);
						raf.close();
					} catch (Exception ex) {
