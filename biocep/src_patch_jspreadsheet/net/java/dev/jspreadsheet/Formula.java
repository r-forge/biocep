...
public class Formula
{
   ...
   Formula(Formula formula, int row, int col) throws ParserException
   {
      ...
      try
      {
         ...
         formulaString = fixRelAddr(formula.formulaString, row - formula.row, col - formula.col);
   ...
   public static boolean isSafe(Formula formula, int rowOff, int colOff)
   {
      // do necessary conversion so the formula string
      // is still right after changing the position
      String newString = fixRelAddr(formula.formulaString, rowOff, colOff);
      ...
      try
      {
         ...
         while (it.hasNext())
         {
            ...
            case Node.REL_ADDR:
               ...
               result = 
                  //	       node.getCol()+col);
                  table.getNumericValueAt(node.getRow() + row, node.getCol() + col);
   ...
   public static String fixRelAddr(String oldFormula, int rowOffset, int colOffset)
   {
      ...
      for (int i = 0; i < oldFormula.length(); i++)
      {
         ...
         if (Character.isUpperCase(c))
         {
            ...
            if (!isAbsAddr && Character.isDigit(oldFormula.charAt(i)))
            {
               ...
               {
                  String col = translateColumn(translateColumn(colName) + colOffset);
                  if (col == null)
                  {
                     return null;
                  }
               ...
               {
                  String row = translateRow(translateRow(rowName) + rowOffset);
                  if (row == null)
                  {
                     return null;
                  }
    ...
    *
    */
   private LinkedList convertParams(final LinkedList tokens) throws ParserException
   {
      if (tokens == null)
      {
         throw error;
      ...
      try
      {
         while (it.hasNext())
         {
            ...
            else if (node.isType(Node.COMMA))
            {
               ...
               while (!param.isType(Node.FUNCTION) || !param.isPending())
               {
                  list.addFirst(param);
            ...
            else if (node.isType(Node.RPAREN))
            {
               ...
               while (!param.isPending() || (!param.isType(Node.FUNCTION) && !param.isType(Node.LPAREN)))
               {
                  list.addFirst(param);
                  param = (Node) stack.removeLast(); //pop();
               }
   ...
   static private Number evalFunction(SpreadsheetTableModel table, Node node, int row, int col) throws ParserException
   {
      ...
      Function func = getFuncHandler(funcName.toUpperCase());
      ...
      if (func == null)
      {
    	  ...
    	  try {
    		  isRFunc=((RLogical)table.getR().evalAndGetObject("exists('"+funcName+"')")).getValue()[0];
    	  } catch (Exception e) {
    		  e.printStackTrace();
    	  }
    	  
    	  if (isRFunc) {
    		  return new FunctionR(funcName).evaluate(table, node, row, col);
    	  } else {    	  
   ...
   private LinkedList toPostfix(LinkedList tokens) throws ParserException
   {
      ...
      while (it.hasNext())
      {
         ...
         case Node.OPERATOR:
            ...
            // the stack
            while (!stack.empty() && !((Node) stack.peek()).isType(Node.LPAREN) && (getPriority((Node) stack.peek()) >= priority))
            {
      ...
      while (cur < input.length())
      {
         ...
         try
         {
            ...
            else if (c == ':')
            {
               ...
               if (prev.isType(Node.REL_ADDR) || prev.isType(Node.ABS_ADDR))
               {
            ...
            if ((cur < input.length()) && (node.isType(Node.REL_ADDR) || node.isType(Node.ABS_ADDR) || node.isType(Node.NUMBER)) && Character.isLetterOrDigit(input.charAt(cur)))
            {
               throwError
               // invalid char
               ("#NAME?");
            ...
            if ((lastToken != null) && lastToken.isType(Node.COLON) && lastToken.isPending())
            {
               if (node.isType(Node.REL_ADDR) || node.isType(Node.ABS_ADDR))
               {
            ...
            if (node.isType(Node.OPERATOR) && (node.getData().equals("+") || node.getData().equals("-")) && ((lastToken == null) || lastToken.isType(Node.LPAREN) || lastToken.isType(Node.COMMA)))
            {
               tokens.add(zero);
            }
    ...
    * @return the string value of the Cell
    */
   public final static String getCellString(int row, int col)
   {
