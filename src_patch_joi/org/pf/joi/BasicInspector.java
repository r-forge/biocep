 ...
 * @since JDK 1.2
 */
public class BasicInspector extends Inspector implements ActionListener, TreeSelectionListener {
	// =========================================================================
	// CONSTANTS
	// =========================================================================
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (((e.getModifiers() & MouseEvent.BUTTON1_MASK) > 0) && (e.getClickCount() == 2)) // Double-click
		{
		...
		if (this.getCurrentElement() != null) {
			try {
				if (where == InspectionWindowController.INSPECT_IN_NEW_WINDOW) {
					...
					Inspector.inspectIn(this.getController(), where, this.getCurrentElement().getName(), this
		...
		if (this.getCurrentElement() != null) {
			try {
				if (where == InspectionWindowController.INSPECT_IN_NEW_WINDOW) {
					...
					Inspector.basicInspectIn(this.getController(), where, this.getCurrentElement().getName(), this
							.getCurrentElement().getValue());
				}
			} catch (Exception ex) {
				ex.printStackTrace(errorDevice);
	...
	protected void updateInformation(Spy spy) {
		...
		if (this.isVisualComponent(value)) {
			...
			this.getContentArea().activateText(this.getValueStringOf(spy), this.isEditableElement(spy),
					spy.isPrimitive());
		}
	} // updateInformation() 
	...
	protected void displayValueModificationError(Exception ex) {
		JOptionPane.showMessageDialog(this.getDialogOwner(), ex.toString(), "Value Modification Error",
