...
public class WorkingDirectoryServlet extends HttpServlet implements ResourceFactory
{
 ...
 public WorkingDirectoryServlet(RKit rkit) {
	 super();
	 _rkit=rkit;
 }
 ...
 {	 
	 pathInContext=pathInContext.substring("/rvirtual/wd".length());
	 try {
		 _resourceBase=Resource.newResource(new File(_rkit.getR().getWorkingDirectory()).toURI().toURL());
